...
export default function Recommend() {
  ...
  }, [mode, budget, useBudget, roll]); // roll forces a re-fetch for surprise rerolls
  ...
  return (
    <>
      ...
      {modeInfo && <p className="hint" style={{ color: "var(--text-dim)", fontSize: 13 }}>{modeInfo[2]}</p>}

      <div className="slider-row">
        <label>
          <input type="checkbox" checked={useBudget} onChange={(e) => setUseBudget(e.target.checked)} /> I
          have about
        </label>
        <input
          type="range"
        ...
        {mode === "surprise" && (
          <button className="btn" onClick={() => setRoll((r) => r + 1)}>
            🎲 Reroll
          </button>
        )}
      </div>
      ...
      <div className="grid">
        {(results ?? []).map((r) => (
          <GameCard key={`${r.game.id}-${roll}`} game={r.game} reason={r.reason} onChanged={() => void load()} />
        ))}
      </div>
    </>
  );
